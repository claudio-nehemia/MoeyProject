...
interface Props {
    bpjskesehatans: PaginatedData<Bpjskesehatan>;
    karyawans: Karyawan[];
    filters: {
        search?: string;
    };
...
export default function Index({ bpjskesehatans, karyawans, filters }: Props) {
    ...
    const [selectedBpjs, setSelectedBpjs] = useState<Bpjskesehatan | null>(null);
    ...
    const handleSearch = (query: string) => {
        ...
        router.get('/bpjs-kesehatan', { search: query }, { preserveState: true, replace: true });
    };
    ...
    const openEditModal = (item: Bpjskesehatan) => {
        setEditMode(true);
    ...
    const handleSubmit: FormEventHandler = (e) => {
        ...
        if (editMode && selectedBpjs) {
            put(`/bpjs-kesehatan/${selectedBpjs.kode_bpjs_kesehatan}`, {
    ...
    const handleDelete = (kode: string) => {
        if (confirm('Are you sure you want to delete this BPJS Kesehatan record?')) {
    ...
    return (
        <>
            ...
            <Sidebar isOpen={sidebarOpen} currentPage="bpjs-kesehatan" onClose={() => setSidebarOpen(false)} />
            ...
            <div className="p-3 lg:ml-60">
                <div className="p-3 mt-20">
                    ...
                    <div className={`flex items-center justify-between mb-4 ${mounted ? 'animate-fadeInUp' : 'opacity-0'}`}>
                        <div>
                            <h1 className="text-2xl font-bold text-stone-900">BPJS Kesehatan</h1>
                            <p className="text-xs text-stone-600">
                                Configure monthly BPJS Kesehatan health insurance allowances or deductions for employees
                            </p>
                        </div>
                        <button
                    ...
                    <div className={`bg-white rounded-xl border border-stone-200 overflow-hidden shadow-sm ${mounted ? 'animate-fadeInUp' : 'opacity-0'}`} style={{ animationDelay: '0.1s' }}>
                        <table className="w-full text-xs text-left text-stone-600 font-sans">
                            <thead className="text-xs text-stone-700 font-semibold bg-gradient-to-r from-stone-50 to-stone-100 border-b border-stone-200">
                                <tr>
                                    ...
                                    <th scope="col" className="px-4 py-2.5">Code (Kode BPJS)</th>
                                    <th scope="col" className="px-4 py-2.5">Employee (Karyawan)</th>
                                    <th scope="col" className="px-4 py-2.5">BPJS Kesehatan Amount (Jumlah)</th>
                            ...
                            <tbody>
                                {bpjskesehatans.data.length === 0 ? (
                                    <tr>
                                        <td colSpan={6} className="px-4 py-8 text-center text-stone-400">
                                            No BPJS Kesehatan records available. Click "Add Record" to configure.
                                        </td>
                                    </tr>
                                ) : (
                                    bpjskesehatans.data.map((item, index) => (
                                        <tr key={item.kode_bpjs_kesehatan} className="bg-white border-b border-stone-100 hover:bg-stone-50/50 transition-colors">
                                            <td className="px-4 py-2.5">
                                                <div className="w-6 h-6 rounded-full bg-gradient-to-br from-amber-400 to-amber-600 flex items-center justify-center text-white font-semibold text-xs shadow-sm">
                                                    {(bpjskesehatans.current_page - 1) * 10 + index + 1}
                                                </div>
                                            </td>
                                            <td className="px-4 py-2.5 font-semibold text-stone-900">{item.kode_bpjs_kesehatan}</td>
                                            <td className="px-4 py-2.5">
                                            ...
                                            <td className="px-4 py-2.5 text-center">
                                                <div className="flex items-center justify-center gap-1.5">
                                                    ...
                                                    <button
                                                        onClick={() => handleDelete(item.kode_bpjs_kesehatan)}
                        ...
                        {/* Pagination Links */}
                        {bpjskesehatans.links && bpjskesehatans.links.length > 3 && (
                            <div className="flex justify-between items-center px-4 py-3 bg-stone-50 border-t border-stone-200">
                                <div className="text-xs text-stone-600">
                                    Showing {bpjskesehatans.data.length} of {bpjskesehatans.total} results
                                </div>
                                <div className="flex gap-1 flex-wrap">
                                    {bpjskesehatans.links.map((link, i) => (
                                        <button
                                            key={i}
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50 font-sans">
                    <div className="w-full max-w-md bg-white rounded-xl shadow-xl overflow-hidden border border-stone-200 animate-fadeInUp">
                        <div className="px-5 py-4 bg-gradient-to-r from-amber-500 to-amber-600 text-white flex justify-between items-center">
                            <h3 className="font-bold text-sm">{editMode ? 'Edit BPJS Kesehatan' : 'Add BPJS Kesehatan'}</h3>
                        ...
                        <form onSubmit={handleSubmit} className="p-5 space-y-4">
                            ...
                            <div>
                                <label className="block text-xs font-semibold text-stone-700 mb-1">BPJS Kesehatan Amount (Jumlah Rp)</label>
                                <input
                                    type="number"
                                    required
                                    value={data.jumlah}
