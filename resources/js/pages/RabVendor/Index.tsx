...
export default function Index({ itemPekerjaans }: Props) {
    ...
    const handleGenerate = (itemPekerjaanId: number) => {
        if (confirm('Generate RAB Vendor dari RAB Internal?')) {
            router.post(`/rab-vendor/${itemPekerjaanId}/generate`);
        }
    };

    const handleDelete = (rabVendorId: number) => {
        if (confirm('Hapus RAB Vendor ini?')) {
            router.delete(`/rab-vendor/${rabVendorId}`);
        }
    };

    return (
        <>
            <Head title="RAB Vendor" />
                ...
                <div className="mt-12 p-3">
                    ...
                    <div className="overflow-hidden bg-white shadow-sm sm:rounded-lg dark:bg-gray-800">
                        <div className="p-6 text-gray-900 dark:text-gray-100">
                            <div className="mb-6">
                                <h3 className="text-lg font-medium">RAB Vendor</h3>
                                <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
                                    RAB dengan harga asli tanpa markup, termasuk aksesoris
                                </p>
                            </div>
                                                    ...
                                                    <td className="whitespace-nowrap px-6 py-4 text-sm">
                                                        {itemPekerjaan.rabVendor ? (
                                                            <span className="inline-flex rounded-full bg-green-100 px-2 text-xs font-semibold leading-5 text-green-800 dark:bg-green-900 dark:text-green-200">
                                                                Sudah Ada ({itemPekerjaan.rabVendor.total_produks} produk)
                                                            </span>
                                                        ) : (
                                                    ...
                                                    <td className="whitespace-nowrap px-6 py-4 text-sm font-medium">
                                                        {itemPekerjaan.rabVendor ? (
                                                            <div className="flex space-x-2">
                                                                <Link
                                                                    href={`/rab-vendor/${itemPekerjaan.rabVendor.id}/show`}
                                                                    className="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300"
                                                                >
                                                                    Lihat RAB
                                                                </Link>
                                                                <button
                                                                    onClick={() => handleDelete(itemPekerjaan.rabVendor!.id)}
