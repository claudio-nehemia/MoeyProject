...
interface Props {
    gajipokoks: PaginatedData<Gajipokok>;
    karyawans: Karyawan[];
    filters: {
        search?: string;
    };
...
export default function Index({ gajipokoks, karyawans, filters }: Props) {
    ...
    const [selectedGaji, setSelectedGaji] = useState<Gajipokok | null>(null);
    ...
    const handleSearch = (query: string) => {
        ...
        router.get('/gaji-pokok', { search: query }, { preserveState: true, replace: true });
    };
    ...
    const openEditModal = (item: Gajipokok) => {
        setEditMode(true);
    ...
    const handleSubmit: FormEventHandler = (e) => {
        ...
        if (editMode && selectedGaji) {
            put(`/gaji-pokok/${selectedGaji.kode_gaji}`, {
    ...
    return (
        <>
            ...
            <Sidebar isOpen={sidebarOpen} currentPage="gaji-pokok" onClose={() => setSidebarOpen(false)} />
            ...
            <div className="p-3 lg:ml-60">
                <div className="p-3 mt-20">
                    ...
                    <div className={`flex items-center justify-between mb-4 ${mounted ? 'animate-fadeInUp' : 'opacity-0'}`}>
                        <div>
                            <h1 className="text-2xl font-bold text-stone-900">Basic Salaries (Gaji Pokok)</h1>
                            <p className="text-xs text-stone-600">
                                Manage basic employee salaries, wage intervals (monthly/daily), and effective dates
                            </p>
                        </div>
                        <button
                    ...
                    <div className={`bg-white rounded-xl border border-stone-200 overflow-hidden shadow-sm ${mounted ? 'animate-fadeInUp' : 'opacity-0'}`} style={{ animationDelay: '0.1s' }}>
                        <table className="w-full text-xs text-left text-stone-600 font-sans">
                            <thead className="text-xs text-stone-700 font-semibold bg-gradient-to-r from-stone-50 to-stone-100 border-b border-stone-200">
                                <tr>
                                    ...
                                    <th scope="col" className="px-4 py-2.5">Type (Jenis Upah)</th>
                                    <th scope="col" className="px-4 py-2.5">Basic Salary (Jumlah)</th>
                            ...
                            <tbody>
                                {gajipokoks.data.length === 0 ? (
                                    <tr>
                                        <td colSpan={7} className="px-4 py-8 text-center text-stone-400">
                                            No basic salary data available. Click "Add Salary" to register.
                                        </td>
                                    </tr>
                                ) : (
                                    gajipokoks.data.map((item, index) => (
                                        <tr key={item.kode_gaji} className="bg-white border-b border-stone-100 hover:bg-stone-50/50 transition-colors">
                                            <td className="px-4 py-2.5">
                                                <div className="w-6 h-6 rounded-full bg-gradient-to-br from-amber-400 to-amber-600 flex items-center justify-center text-white font-semibold text-xs shadow-sm">
                                                    {(gajipokoks.current_page - 1) * 10 + index + 1}
                                                </div>
                                            </td>
                                            <td className="px-4 py-2.5 font-semibold text-stone-900">{item.kode_gaji}</td>
                                            <td className="px-4 py-2.5">
                                            ...
                                            <td className="px-4 py-2.5 text-center">
                                                <div className="flex items-center justify-center gap-1.5">
                                                    ...
                                                    <button
                                                        onClick={() => handleDelete(item.kode_gaji)}
                        ...
                        {/* Pagination Links */}
                        {gajipokoks.links && gajipokoks.links.length > 3 && (
                            <div className="flex justify-between items-center px-4 py-3 bg-stone-50 border-t border-stone-200">
                                <div className="text-xs text-stone-600">
                                    Showing {gajipokoks.data.length} of {gajipokoks.total} results
                                </div>
                                <div className="flex gap-1 flex-wrap">
                                    {gajipokoks.links.map((link, i) => (
                                        <button
                                            key={i}
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50 font-sans">
                    <div className="w-full max-w-md bg-white rounded-xl shadow-xl overflow-hidden border border-stone-200 animate-fadeInUp">
                        <div className="px-5 py-4 bg-gradient-to-r from-amber-500 to-amber-600 text-white flex justify-between items-center">
                            <h3 className="font-bold text-sm">{editMode ? 'Edit Basic Salary' : 'Add Basic Salary'}</h3>
                        ...
                        <form onSubmit={handleSubmit} className="p-5 space-y-4">
                            {!editMode ? (
                                ...
                                <div>
                                    ...
                                    <input
                                        type="text"
                                        disabled
                                        value={`${selectedGaji?.nama_karyawan} (${selectedGaji?.nik_show})`}
                            ...
                            <div>
                                <label className="block text-xs font-semibold text-stone-700 mb-1">Wage Interval (Jenis Upah)</label>
                                <select
                                    required
                                    value={data.jenis_upah}
                                    onChange={e => setData('jenis_upah', e.target.value as 'Bulanan' | 'Harian')}
                                    className="w-full px-3 py-2 text-sm border border-stone-300 rounded-lg focus:outline-none focus:border-amber-500 bg-white"
                                >
                                    <option value="Bulanan">Bulanan</option>
                                    <option value="Harian">Harian</option>
                                </select>
                                {errors.jenis_upah && <p className="text-red-500 text-xs mt-1">{errors.jenis_upah}</p>}
                            </div>

                            <div>
                                <label className="block text-xs font-semibold text-stone-700 mb-1">Basic Salary (Jumlah Rp)</label>
                                <input
                                    type="number"
                                    required
                                    value={data.jumlah}
