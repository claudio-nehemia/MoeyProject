...
export default function Index({ slipgajis, slipgaji_harian, karyawans, list_bulan, tahun_harian }: Props) {
    ...
    const openEditB = (item: Slipgaji) => {
        ...
        formB.setData({
            bulan: item.bulan,
            tahun: item.tahun,
            status: (item.status === '1' || item.status === true || String(item.status) === '1') ? '1' : '0',
    ...
    const handleToggleStatusB = (item: Slipgaji) => {
        const isPublished = item.status === '1' || item.status === true || String(item.status) === '1';
                                            ...
                                            <tr key={item.kode_slip_gaji} className="bg-white border-b border-stone-100 hover:bg-stone-50/50 transition-colors">
                                                ...
                                                <td className="px-4 py-2.5">
                                                     <span className={`inline-flex items-center px-2 py-0.5 rounded text-xxs font-semibold ${
                                                         (item.status === '1' || item.status === true || String(item.status) === '1')
                                                             ? 'bg-green-50 text-green-700 border border-green-200'
                                                             : 'bg-yellow-50 text-yellow-700 border border-yellow-200'
                                                     }`}>
                                                         {(item.status === '1' || item.status === true || String(item.status) === '1') ? 'Published' : 'Draft'}
                                                     </span>
                                                 </td>
