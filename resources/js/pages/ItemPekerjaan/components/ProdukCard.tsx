...
export default function ProdukCard({
    ...
    return (
        <div className="bg-slate-50 rounded-xl border border-slate-200 overflow-hidden shadow-sm">
            {/* Produk Header */}
            <div className={`bg-gradient-to-r ${isExpanded ? 'from-purple-500 to-purple-600' : 'from-slate-600 to-slate-700'} px-4 py-3 flex justify-between items-center transition-all duration-300`}>
                <div className="flex items-center gap-3 flex-1">
                    ...
                    <h4 className="text-white font-medium flex items-center gap-2">
                        Produk #{produkIndex + 1}: {selectedProduk?.nama_produk || 'Belum Dipilih'}
                        {produk.id && (
                            <span className="text-[10px] bg-white/20 px-2 py-0.5 rounded uppercase tracking-wider">ID: {produk.id}</span>
                        )}
                        {!isExpanded && produk.quantity > 0 && (
                            <span className="text-[10px] bg-emerald-500/40 px-2 py-0.5 rounded uppercase tracking-wider">Qty: {produk.quantity}</span>
                        )}
                    </h4>
                </div>
                <button
                    ...
                    className="bg-red-500/20 text-red-100 px-3 py-1.5 rounded-lg text-xs font-medium hover:bg-red-500/40 transition-colors flex items-center gap-1"
                >
                        ...
                        <div className="grid grid-cols-3 gap-3">
                            <div>
                                <input
                                    ...
                                    className="w-full border border-slate-300 rounded-lg px-3 py-2.5 focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition-all text-sm"
                                />
                                <p className="mt-0.5 text-[10px] text-slate-500 uppercase tracking-wider text-center">Panjang</p>
                            </div>
                            <div>
                                <input
                                    type="number"
                                    ...
                                    className="w-full border border-slate-300 rounded-lg px-3 py-2.5 focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition-all text-sm"
                                />
                                <p className="mt-0.5 text-[10px] text-slate-500 uppercase tracking-wider text-center">Lebar</p>
                            </div>
                            <div>
                                <input
                                    type="number"
                                    ...
                                    className="w-full border border-slate-300 rounded-lg px-3 py-2.5 focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition-all text-sm"
                                />
                                <p className="mt-0.5 text-[10px] text-slate-500 uppercase tracking-wider text-center">Tinggi</p>
                            </div>
                        </div>
                    </div>
                            ...
                            <div className="bg-gradient-to-r from-amber-500 to-orange-500 px-4 py-2.5">
                                <div className="flex justify-between items-center">
                                    <h5 className="text-white font-medium text-sm flex items-center gap-2">
                                        ...
                                        Pilih Bahan Baku 
                                        <span className="text-amber-200 text-xs font-normal ml-1">
                                    ...
                                    <div className="flex gap-2">
                                        <button
                                            ...
                                            className="bg-white/20 text-white px-2 py-1 rounded text-[10px] font-bold uppercase hover:bg-white/30 transition-colors"
                                        >
                                            Pilih Semua
                                        </button>
                                        <button
                                            ...
                                            className="bg-white/10 text-white px-2 py-1 rounded text-[10px] font-bold uppercase hover:bg-white/20 transition-colors"
                                        >
                                            Clear
                                        </button>
                                    </div>
                    ...
                    <div className="bg-white rounded-xl border border-slate-200 p-3">
                        <div className="flex justify-between items-center mb-3">
                            ...
                            <button
                                ...
                                className="bg-emerald-500 text-white px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase hover:bg-emerald-600 transition-colors flex items-center gap-1"
                            >
                                <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        {produk.jenisItems.filter(j => j.jenis_item_name?.toLowerCase() !== 'bahan baku').length === 0 ? (
                            <div className="bg-slate-50 rounded-lg border-2 border-dashed border-slate-200 p-4 text-center">
                                <p className="text-slate-400 text-xs italic">Belum ada kategori. Klik "Tambah Kategori"</p>
                            </div>
                        ) : (
