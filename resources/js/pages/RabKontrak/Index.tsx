...
export default function Index({ itemPekerjaans }: Props) {
    ...
    const handleGenerate = (itemPekerjaanId: number) => {
        if (confirm('Generate RAB Kontrak dari RAB Internal?')) {
            router.post(`/rab-kontrak/${itemPekerjaanId}/generate`);
        }
    };

    return (
        <>
            <Head title="RAB Kontrak" />
                ...
                <div className="mt-12 p-3">
                    ...
                    <div className="overflow-hidden bg-white shadow-sm sm:rounded-lg dark:bg-gray-800">
                        <div className="p-6 text-gray-900 dark:text-gray-100">
                            <div className="mb-6">
                                <h3 className="text-lg font-medium">Daftar Item Pekerjaan - RAB Kontrak</h3>
                                <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
                                    Kelola RAB Kontrak untuk setiap item pekerjaan (Hanya muncul jika RAB Internal sudah ada)
                                </p>
                            </div>
                                                    ...
                                                    <td className="whitespace-nowrap px-6 py-4 text-sm">
                                                        {itemPekerjaan.rabKontrak ? (
                                                            <span className="inline-flex rounded-full bg-green-100 px-2 text-xs font-semibold leading-5 text-green-800 dark:bg-green-900 dark:text-green-200">
                                                                Sudah Ada ({itemPekerjaan.rabKontrak.total_produks} produk)
                                                            </span>
                                                        ) : (
                                                    ...
                                                    <td className="whitespace-nowrap px-6 py-4 text-sm font-medium">
                                                        {itemPekerjaan.rabKontrak ? (
                                                            <div className="flex space-x-2">
                                                                <Link
                                                                    href={`/rab-kontrak/${itemPekerjaan.rabKontrak.id}/show`}
